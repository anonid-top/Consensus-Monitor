...
class AioHttpCalls:
    ...
    def __init__(
        ...
        """
        Initializes the AioHttpCalls instance.

        Args:
            rpc (str): The RPC endpoint URL.
            session (aiohttp.ClientSession, optional): An existing aiohttp session to use. If None, a new session will be created.
    ...
    async def handle_abci_request(self, callback, hex_data, path, prove=False) -> bytes:
        try:
            payload = {
                "jsonrpc": "2.0",
                "method": "abci_query",
                "params": {"path": path, "data": hex_data, "prove": prove},
                "id": -1,
            }
            ...
            async with self.session.get(
                ...
                if response.status == 200:
                    ...
                    if code == 0:
                        ...
                        logger.error(
                            f"ABCI retuned {code} code. Payload: {payload}. ABCI log: {abci_error_log}"
                        )
                else:
                    logger.error(
                        f"Request to {self.rpc} failed with status code {response.status}. Payload: {payload}"
                    )

        except aiohttp.ClientError as e:
            logger.error(
                f"Issue with making request to {self.rpc}. Payload: {payload}. {e}"
            )
        except TimeoutError as e:
            logger.error(
                f"Issue with making request to {self.rpc}. Payload: {payload}. TimeoutError: {e}"
            )
        except Exception as e:
            logger.error(
                f"An unexpected error occurred while making request to {self.rpc}: {e}"
            )
            traceback.print_exc()
    ...
    async def get_validators(
        self,
        status: Literal[
            "BOND_STATUS_BONDED", "BOND_STATUS_UNBONDED", "BOND_STATUS_UNBONDING", None
        ],
        limit: int = 1000,
        offset=None,
        count_total=False,
        ...
        logger.debug(f"Requesting {self.rpc}/cosmos.staking.v1beta1.Query/Validators")
        ...
        return await self.handle_abci_request(
    ...
    async def get_upgrade_info(self):
        logger.debug(f"Requesting {self.rpc}/cosmos.upgrade.v1beta1.Query/CurrentPlan")
    ...
    async def get_block(self, height: int) -> str:
        url = f"{self.rpc}/block?height={str(height)}"
        logger.debug(f"Requesting {url}")
